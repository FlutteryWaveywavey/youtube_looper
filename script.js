...
function setLoopPointEnabled(enabled) {
...
function loadVideoByUrl() {
  ...
  player = new YT.Player("player", {
    ...
    events: {
      onReady: () => setLoopPointEnabled(true),
      onError: (e) => {
        console.error("YT Player error:", e);
        alert("재생 오류가 발생했습니다. 영상이 임베드 허용인지 확인해 주세요.");
        setLoopPointEnabled(false);
      }
    }
  });
}
...
function setLoopPoint(){
    ...
    document.getElementById("bookmarkInfo").textContent = `루프포인트: ${formatTime(currentTime)}`;
    loopPointTime = currentTime;
}
...
// Set loop point button
document.getElementById("setBookmarkBtn").addEventListener("click", setLoopPoint);
// Set loop button
